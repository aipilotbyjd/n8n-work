...
export class SLOService implements OnModuleInit {
  ...
  private async evaluateSLO(name: string, definition: SLODefinition): Promise<void> {
  ...
  private async calculateCompliance(sli: SLIDefinition, objective: SLOObjective): Promise<number> {
    try {
  ...
  private async calculateErrorBudget(status: SLOStatus, definition: SLODefinition): Promise<void> {
  ...
  private determineSLOStatus(status: SLOStatus): 'healthy' | 'warning' | 'critical' {
  ...
  private async checkAlerts(name: string, definition: SLODefinition, status: SLOStatus): Promise<void> {
  ...
  private shouldFireAlert(definition: SLODefinition, status: SLOStatus): boolean {
    ...
    const hasCriticalBreach = definition.objectives.some((obj, i) => 
      obj.severity === 'critical' && status.objectives[i].status === 'breached'
    );

    return hasCriticalBreach;
  }

  private async fireAlert(name: string, definition: SLODefinition, status: SLOStatus): Promise<void> {
    const alert = {
  ...
  private async resolveAlert(name: string, definition: SLODefinition, status: SLOStatus): Promise<void> {
    const alert = {
  ...
  private buildAlertMessage(definition: SLODefinition, status: SLOStatus): string {
    const breachedObjectives = definition.objectives.filter((obj, i) => 
  ...
  private getAlertSeverity(definition: SLODefinition, status: SLOStatus): 'critical' | 'warning' | 'info' {
    const hasCriticalBreach = definition.objectives.some((obj, i) => 
      obj.severity === 'critical' && status.objectives[i].status === 'breached'
    );
