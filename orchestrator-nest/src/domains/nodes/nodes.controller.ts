...
export class NodesController {
  ...
  @ApiQuery({ name: 'search', required: false, description: 'Search node types by name or keywords' })
  @ApiQuery({ name: 'isBuiltIn', required: false, type: Boolean, description: 'Filter built-in vs custom nodes' })
  ...
  async findOneNode(
    @Param('id') id: string,
    @Tenant() tenantId: string,
    @Query('includeVersions', new DefaultValuePipe(false), ParseBoolPipe) includeVersions = false,
  ): Promise<NodeResponseDto> {
  ...
  @ApiBadRequestResponse({ description: 'Invalid update data or built-in node' })
  @ApiNotFoundResponse({ description: 'Node type not found' })
  @ApiUnauthorizedResponse({ description: 'Authentication required' })
  async updateNode(
    @Param('id') id: string,
  ...
  @ApiOperation({
    ...
    description: 'Soft-deletes a custom node type (built-in nodes cannot be deleted).',
  })
  @ApiParam({ name: 'id', description: 'Node type UUID' })
  @ApiNoContentResponse({ description: 'Node type deleted successfully' })
  @ApiBadRequestResponse({ description: 'Cannot delete built-in node or node in use' })
  @ApiNotFoundResponse({ description: 'Node type not found' })
  @ApiUnauthorizedResponse({ description: 'Authentication required' })
  ...
  async createNodeVersion(
    ...
    return this.nodesService.createNodeVersion(id, versionData, user.id, tenantId);
  }
  ...
  async testNode(
    ...
    return this.nodesService.testNode(id, testData.input, testData.configuration, tenantId);
  }
  ...
  async installPluginPackage(
    ...
    return this.nodesService.installPluginPackage(createPluginDto, user.id, tenantId);
  }
  ...
  @ApiNoContentResponse({ description: 'Plugin package uninstalled successfully' })
  @HttpCode(HttpStatus.NO_CONTENT)
  async uninstallPluginPackage(
    ...
    return this.nodesService.uninstallPluginPackage(packageId, user.id, tenantId);
  }
  ...
  @ApiQuery({ name: 'search', required: false, description: 'Search packages by name or description' })
