...
import { AIAgent, AIAgentStatus, AIAgentType } from './entities/ai-agent.entity';
import { AIAgentExecution, AIExecutionStatus } from './entities/ai-agent-execution.entity';
...
export class AIAgentsService {
  ...
  async create(
    ...
    this.logger.log(`Creating AI agent: ${createAIAgentDto.name} for tenant: ${tenantId}`);
  ...
  async findAll(tenantId: string, filters?: AIAgentFilterDto): Promise<AIAgent[]> {
  ...
  async findOne(id: string, tenantId: string, includeExecutions = false): Promise<AIAgent> {
  ...
  async update(
    ...
    if (updateAIAgentDto.modelConfig) {
      try {
        ...
        this.logger.error(`Failed to reinitialize agent ${savedAgent.id}:`, error);
        savedAgent.status = AIAgentStatus.ERROR;
        await this.aiAgentRepository.save(savedAgent);
      }
    }
  ...
  async remove(id: string, tenantId: string, userId: string): Promise<void> {
    ...
    if (activeExecutions > 0) {
      throw new BadRequestException('Cannot delete agent with active executions');
    }
  ...
  async execute(
    ...
    if (!agent.canExecute()) {
      throw new BadRequestException(`AI agent ${id} is not available for execution`);
    }
    ...
    const execution = this.executionRepository.create({
      ...
      timeoutSeconds: executeDto.timeoutSeconds || agent.resourceRequirements.maxConcurrency || 300,
  ...
  async getExecution(executionId: string, tenantId: string): Promise<AIAgentExecution> {
    ...
    if (!execution) {
      throw new NotFoundException(`AI execution with ID ${executionId} not found`);
    }
  ...
  async cancelExecution(executionId: string, tenantId: string): Promise<AIAgentExecution> {
    const execution = await this.getExecution(executionId, tenantId);

    if (execution.status !== AIExecutionStatus.RUNNING && 
        execution.status !== AIExecutionStatus.PENDING) {
      throw new BadRequestException('Cannot cancel execution that is not running or pending');
    }
  ...
  async retryExecution(executionId: string, tenantId: string): Promise<AIAgentExecution> {
    const execution = await this.getExecution(executionId, tenantId);
    ...
    this.executeAgentAsync(execution.agent, savedExecution).catch((error) => {
      this.logger.error(`Retry execution failed for execution ${executionId}:`, error);
    });
  ...
  async getAgentStats(agentId: string, tenantId: string, days = 30) {
    ...
    const successfulExecutions = executions.filter(e => e.status === AIExecutionStatus.COMPLETED).length;
    const failedExecutions = executions.filter(e => e.status === AIExecutionStatus.FAILED).length;
  ...
  private async validateModelConfig(modelConfig: any): Promise<void> {
    if (!modelConfig.provider || !modelConfig.model) {
      throw new BadRequestException('Model provider and model name are required');
    }
  ...
  private async executeAgentAsync(agent: AIAgent, execution: AIAgentExecution): Promise<void> {
    try {
