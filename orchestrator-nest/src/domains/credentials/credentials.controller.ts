...
export class CredentialsController {
  ...
  @ApiOperation({
    ...
    description:
      "Creates a new credential with encrypted storage of sensitive data.",
  })
  @ApiCreatedResponse({
  ...
  @ApiBadRequestResponse({
    description: "Invalid credential data or validation failed",
  })
  ...
  @ApiQuery({
    ...
    type: Boolean,
    description:
      "Include decrypted credential data in response (requires admin permissions)",
    example: false,
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves all available credential types that can be used to create credentials.",
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description:
      "Tests whether the credential can successfully authenticate with its target service.",
  })
  @ApiParam({ name: "id", description: "Credential UUID" })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description:
      "Initiates an OAuth 2.0 authorization flow for the specified credential.",
  })
  @ApiParam({ name: "id", description: "Credential UUID" })
  @ApiOkResponse({
  ...
  @ApiBadRequestResponse({
    description: "Credential does not support OAuth or invalid request",
  })
  @ApiNotFoundResponse({ description: "Credential not found" })
  @ApiUnauthorizedResponse({ description: "Authentication required" })
  async startOAuth(
    ...
    return this.credentialsService.startOAuthFlow(
      id,
      tenantId,
      body.redirectUrl,
    );
  ...
  @ApiOperation({
    ...
    description:
      "Completes the OAuth 2.0 flow by exchanging the authorization code for access tokens.",
  })
  @ApiOkResponse({
  ...
  @ApiBadRequestResponse({
    description: "Invalid OAuth callback data or expired state",
  })
  @ApiUnauthorizedResponse({ description: "Authentication required" })
  async oauthCallback(
    ...
    if (oauthCallbackDto.error) {
      throw new Error(
        `OAuth error: ${oauthCallbackDto.error_description || oauthCallbackDto.error}`,
      );
    }
  ...
  @ApiBadRequestResponse({
    description: "Unable to refresh token or token not found",
  })
  @ApiNotFoundResponse({ description: "Credential not found" })
  @ApiUnauthorizedResponse({ description: "Authentication required" })
