...
export class BillingController {
  ...
  @ApiResponse({
    ...
    description:
      "Invalid request data or tenant already has active subscription",
  })
  ...
  async getBillingDashboard(@GetCurrentTenant() tenantId: string) {
    const subscription =
      await this.billingService.getSubscriptionByTenant(tenantId);
  ...
  async getUsageReport(
    ...
    const usageCheck = await this.billingService.checkUsageLimits(
      tenantId,
      mockUsage,
    );
