...
import { UsePipes, ValidationPipe, Logger, OnModuleDestroy, OnModuleInit } from '@nestjs/common';
...
export type SubscriptionType = 'workflow' | 'execution' | 'logs' | 'metrics' | 'tenant_activity';
...
export class WorkflowWebSocketGateway
    ...
    onModuleInit() {
        ...
        this.cleanupTimer = setInterval(() => this.cleanupInactiveConnections(), 30_000);
    ...
    async handleConnection(client: AuthenticatedSocket) {
        try {
            ...
            this.logger.log(`Client connected cid=${client.cid} tenant=${client.tenantId} user=${client.userId}`);
    ...
    async subscribe(
        ...
        try {
            if (!client.tenantId || !client.userId) {
                return client.emit('error', { code: 'UNAUTHORIZED', message: 'Authenticate first' });
            }

            if (!this.isValidSubscriptionType(request.type)) {
                return client.emit('error', { code: 'BAD_REQUEST', message: 'Invalid subscription type' });
            }

            const allowed = await this.checkSubscriptionPermission(client.tenantId, client.userId, request);
            if (!allowed) {
                return client.emit('error', { code: 'FORBIDDEN', message: 'Not allowed for this resource' });
            }

            const key = this.generateSubscriptionKey(request, client.tenantId);

            // attach client to subscription
            if (!this.subscriptions.has(key)) this.subscriptions.set(key, new Set<string>());
            this.subscriptions.get(key)!.add(client.cid!);

            if (!this.clientSubscriptions.has(client.cid!)) this.clientSubscriptions.set(client.cid!, new Set());
            ...
            client.emit('error', { code: 'SUBSCRIBE_ERROR', message: 'Failed to subscribe' });
        }
    }
    ...
    async unsubscribe(
        ...
        try {
            ...
            client.emit('error', { code: 'UNSUBSCRIBE_ERROR', message: 'Failed to unsubscribe' });
        }
    }
    ...
    private extractToken(client: Socket): string {
        ...
        const hAuth = client.handshake.headers['authorization'] as string | undefined;
