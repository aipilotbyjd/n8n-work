export const configuration = () => ({
  ...
  cache: {
    ...
    redis: {
      host:
        process.env.CACHE_REDIS_HOST || process.env.REDIS_HOST || "localhost",
      port:
  ...
  messageQueue: {
    url:
      process.env.RABBITMQ_URL || "amqp://n8nwork:n8nwork_dev@localhost:5672",
    exchanges: {
  ...
  // External Services
  services: {
    executionEngineType: process.env.EXECUTION_ENGINE_TYPE || "nest",
  ...
  limits: {
    ...
    maxConcurrentExecutions:
      parseInt(process.env.MAX_CONCURRENT_EXECUTIONS, 10) || 100,
  ...
  notifications: {
    defaultChannels: (
      process.env.DEFAULT_NOTIFICATION_CHANNELS || "email,slack"
    ).split(","),
